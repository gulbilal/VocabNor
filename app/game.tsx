...
export default function GameScreen() {
  ...
  const gameComplete = score === words.length;
  ...
  function selectEnglishWord(id: number) {
    if (selectedNorwegianId === null || isCheckingAnswer) {
      return;
    }
      ...
      {roundComplete && (
        <View style={styles.completeContainer}>
          <Text style={styles.completeText}>
            {gameComplete ? "All words complete!" : "Round complete!"}
          </Text>
          <Pressable
            style={styles.newRoundButton}
            onPress={gameComplete ? restartGame : startNextRound}
          >
            <Text style={styles.newRoundButtonText}>
              {gameComplete ? "Restart Game" : "Start Next Round"}
            </Text>
          </Pressable>
        </View>
      )}
